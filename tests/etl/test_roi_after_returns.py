...
def test_roi_after_returns(pg_pool, refresh_mvs):
    ...
    with engine.begin() as conn:
        conn.execute(
            text(
                "INSERT INTO vendors(id, name) VALUES (1,'ACME') ON CONFLICT DO NOTHING"
            )
        )
        conn.execute(text("INSERT INTO products(asin) VALUES ('A1')"))
        conn.execute(
            text("INSERT INTO vendor_prices(vendor_id, sku, cost) VALUES (1,'A1',10)")
        )
        conn.execute(
            text(
        ...
        conn.execute(
            text("INSERT INTO keepa_offers(asin, buybox_price) VALUES ('A1',20)")
        )
        conn.execute(
            text(
