...
def _setup_db():
    ...
    with engine.begin() as conn:
        ...
        insert_keepa = (
            "INSERT INTO keepa_offers(asin, buybox_price) VALUES (:asin,:price) "
        ...
        insert_fee = (
            ...
            "VALUES (:asin,1,1,1,'EUR') ON CONFLICT (asin) DO UPDATE SET updated_at=CURRENT_TIMESTAMP"
        )
        conn.execute(
            text(
        ...
        conn.execute(
            text(
                "INSERT INTO products(asin, title, category, weight_kg) VALUES ('A1','t1','cat',1) "
                "ON CONFLICT (asin) DO UPDATE SET title=EXCLUDED.title"
            )
        )
        conn.execute(
            text(
                "INSERT INTO products(asin, title, category, weight_kg) VALUES ('A2','t2','cat',1) "
                "ON CONFLICT (asin) DO UPDATE SET title=EXCLUDED.title"
            )
        )
        conn.execute(
        ...
        conn.execute(
            text(
                "INSERT INTO vendor_prices(vendor_id, sku, cost) VALUES (1,'A1',10) "
                "ON CONFLICT (vendor_id, sku) DO UPDATE SET cost=EXCLUDED.cost"
            )
        )
        conn.execute(
            text(
                "INSERT INTO vendor_prices(vendor_id, sku, cost) VALUES (1,'A2',25) "
                "ON CONFLICT (vendor_id, sku) DO UPDATE SET cost=EXCLUDED.cost"
            )
        )
        conn.execute(
