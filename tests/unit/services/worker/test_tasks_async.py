...
def test_alertbot_run(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(tasks, "_evaluate_alerts_sync", lambda: {"status": "ok"})
    assert tasks.alertbot_run()["status"] == "ok"
