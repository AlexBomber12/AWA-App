...
async def _local_llm(prompt: str, temp: float, max_toks: int) -> str:
    async with httpx.AsyncClient(timeout=60) as cli:
        r = await cli.post(LOCAL_URL, json={"prompt": prompt, "temperature": temp, "max_tokens": max_toks})
        r.raise_for_status()
...
async def _openai_llm(prompt: str, temp: float, max_toks: int) -> str:
    ...
    rsp = await openai.ChatCompletion.acreate(
        model=OPENAI_MODEL, messages=[{"role": "user", "content": prompt}], temperature=temp, max_tokens=max_toks
    )
