...
async def ready():
    ...
    try:
        ...
        return {"status": "fail", "reason": "redis"}, status.HTTP_503_SERVICE_UNAVAILABLE
    ...
    if celery_app:
        try:
            ...
            pong = i.ping() or {}
            if not pong:
                return {"status": "fail", "reason": "celery-ping"}, status.HTTP_503_SERVICE_UNAVAILABLE
        except Exception:
            return {"status": "fail", "reason": "celery-exc"}, status.HTTP_503_SERVICE_UNAVAILABLE
